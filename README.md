...
| 遥测开关用户可控 | **是**，设置 `ZCODE_TELEMETRY_ENABLED=false` 即可关闭（官方版硬编码为 true） |
...
[Sync upstream](.github/workflows/sync-upstream.yml) 每天自动检查上游更新：能自动合并且 build、类型检查都通过就直接推送到 main；有冲突或检查失败时开一个 PR，按 PR 里的说明手动处理。
